...
namespace WpfApp1.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private void Tasks_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (TaskItem item in e.NewItems) item.PropertyChanged += Task_PropertyChanged;
            }
            if (e.OldItems != null)
            {
                foreach (TaskItem item in e.OldItems) item.PropertyChanged -= Task_PropertyChanged;
            }
        }
